 ...
 * This function provides a thorough cleanup of an AVPacket to prevent memory
 * leaks
 *
...
void safe_avformat_cleanup(AVFormatContext **ctx_ptr) {
  // CRITICAL FIX: Add comprehensive safety checks to prevent segmentation
  // faults
  if (!ctx_ptr) {
  ...
  *ctx_ptr = NULL;
  ...
  if (ctx_to_close->nb_streams > 0 && ctx_to_close->streams) {
    ...
    log_debug("Closing input context with %d streams",
              ctx_to_close->nb_streams);
 ...
 * This function ensures all resources associated with an AVFormatContext are
 * properly freed
 *
 ...
 */
void comprehensive_ffmpeg_cleanup(AVFormatContext **input_ctx,
                                  AVCodecContext **codec_ctx, AVPacket **packet,
                                  AVFrame **frame) {
  ...
  // Clean up input context with special handling for parsers and internal
  // buffers
      ...
      for (unsigned int i = 0; i < ctx->nb_streams; i++) {
        if (ctx->streams[i]) {
          ...
          if (ctx->streams[i]->codecpar) {
            ...
            // CRITICAL FIX: Explicitly clean up any internal allocations made
            // by avcodec_parameters_from_context This addresses the memory leak
            // shown in Valgrind
...
// Note: FFmpeg 5.1+ uses ch_layout, older versions use channel_layout
#if LIBAVFORMAT_VERSION_INT >= AV_VERSION_INT(59, 0, 100)
              // FFmpeg 5.0+: Use new ch_layout API
              av_channel_layout_uninit(&codecpar->ch_layout);
#else
              // FFmpeg 4.x: Use old channel_layout
              codecpar->channel_layout = 0;
#endif

              // ENHANCED FIX: Create a temporary codec context to force cleanup
              // of internal allocations This is a workaround for FFmpeg's
              // internal memory management issues
              ...
              if (codec) {
                ...
                if (temp_ctx) {
                  ...
                  if (ret >= 0) {
                    // Now copy back to parameters - this will reallocate and
                    // clean up any leaks
 ...
 * This function performs a periodic reset of FFmpeg resources to prevent memory
 * growth It should be called periodically during long-running operations
 *
 ...
 */
int periodic_ffmpeg_reset(AVFormatContext **input_ctx_ptr, const char *url,
                          int protocol) {
  ...
  av_dict_set(&options, "stimeout", "5000000",
              0); // 5 second timeout in microseconds
  av_dict_set(&options, "reconnect", "1", 0); // Auto reconnect
  av_dict_set(&options, "reconnect_streamed", "1", 0);
  av_dict_set(&options, "reconnect_delay_max", "2",
              0); // Max 2 seconds between reconnects
  ...
  while (*p) {
    ...
    *p = '\0';
    ...
    if (mkdir(path_copy, 0755) != 0 && errno != EEXIST) {
      log_error("Failed to create directory %s: %s", path_copy,
                strerror(errno));
      free(path_copy);
      return -1;
    }
 ...
 * Note: This is a simplified version that only sets permissions on the
 * directory itself For full recursive chmod, we would need to walk the
 * directory tree
 */
...
int ffmpeg_encode_jpeg(const unsigned char *frame_data, int width, int height,
  ...
  sws_ctx = sws_getContext(width, height, src_pix_fmt, width, height,
                           codec_ctx->pix_fmt, SWS_BILINEAR, NULL, NULL, NULL);
  if (!sws_ctx) {
  ...
  sws_scale(sws_ctx, src_data, src_linesize, 0, height, frame->data,
            frame->linesize);
  ...
  if (!outfile) {
    log_error("Failed to open output file %s: %s", output_path,
              strerror(errno));
    ret = -1;
    goto cleanup;
  }
  ...
  log_debug("Encoded JPEG: %dx%d, %d bytes -> %s", width, height, pkt->size,
            output_path);
  ret = 0;
...
/**
 * Concatenate multiple TS segments into a single MP4 file using FFmpeg
 * libraries
 */
int ffmpeg_concat_ts_to_mp4(const char **segment_paths, int segment_count,
  ...
  for (int seg = 0; seg < segment_count; seg++) {
    ...
    if (seg > 0) {
      ret = avformat_open_input(&input_contexts[seg], segment_paths[seg], NULL,
                                NULL);
      if (ret < 0) {
      ...
      if (ret < 0) {
        log_warn("Failed to find stream info for segment %s, skipping",
                 segment_paths[seg]);
    ...
    // Update offsets for next segment (add a small gap to avoid timestamp
    // collision)
    for (int i = 0; i < output_stream_count; i++) {
      AVStream *out_stream = output_ctx->streams[i];
      int64_t gap =
          av_rescale_q(1, (AVRational){1, 90000}, out_stream->time_base);
  ...
  log_info("Successfully concatenated %d segments to %s", segment_count,
           output_path);
  ret = 0;
